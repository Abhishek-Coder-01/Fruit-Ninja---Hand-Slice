...
(function () {
    ...
    function activatePower(emoji,cx,cy,basePts) {
        ...
        switch(emoji){
            ...
            case '2️⃣': { const dMs=randomPowerDurationMs(); doubleScore=true; clearTimeout(doubleScoreTimer); doubleScoreTimer=setTimeout(()=>{doubleScore=false;},dMs); splashText='2️⃣ DOUBLE SCORE!'; splashColor='#ff44ff'; addPowerBadge('double','2️⃣ DOUBLE','#ff44ff',dMs); break; }
            ...
            case '❄️': pts=basePts+40; splashText='❄️ +40 BONUS!'; splashColor='#aaeeff'; addPowerBadge('ice','❄️ +40','#aaeeff',800); break;
    ...
    // ── SPAWN ────────────────────────────────────────────────────────────────────
    function spawnItem(diff=getDifficultyState()) {
        if (!active) return;
    ...
    function startSpawning() {
        ...
        for(let i=0;i<2;i++) setTimeout(()=>{if(!active)return;const d=getDifficultyState();if(fruits.filter(f=>!f.sliced).length+bombs.length<d.maxFruits)spawnItem(d);},i*260);
        spawnTimer=setInterval(()=>{
            if(!active)return;
            const d=getDifficultyState(),now=performance.now();
            if(fruits.filter(f=>!f.sliced).length+bombs.length>=d.maxFruits)return;
            if(now-lastSpawnAt<d.spawnRate)return;
            spawnItem(d); lastSpawnAt=now;
        },120);
    }
